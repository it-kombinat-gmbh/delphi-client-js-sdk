...
import { SessionClient, type BrowserAudioEvent } from "./SessionClient";
...
export class ReadAloudCapabilityNotFoundError extends Error {
  ...
  constructor(endpointId: string) {
    super(
      `Endpoint ${endpointId} has no readAloud browser-action capability configured.`,
    );
  }
}

function supportedInteractionModes(
...
export class DelphiClient {
  ...
  async getSessionToken(
    ...
    if (!sessionTokenUrl && !apiDomain)
      throw new Error("apiDomain not configured");
    ...
    if (!response.ok) {
      ...
      throw new Error(
        err.error || `Failed to get session token: ${response.status}`,
      );
    }
    ...
    if (!response.ok) {
      ...
      const message =
        typeof err.error === "string"
          ? err.error
          : err.error?.message ||
            `Failed to upgrade session: ${response.status}`;
    ...
    if (!response.ok) {
      ...
      const message =
        typeof err.error === "string"
          ? err.error
          : err.error?.message ||
            `Failed to downgrade session: ${response.status}`;
      throw new Error(message);
    }
  ...
  async openSessionWithToken(
    ...
    let voiceGateways:
      { telproDomain: string; webrtcGatewayUrl: string } | undefined;
    ...
    const wsUrl = this._config.apiDomain
      ? `wss://${this._config.apiDomain}`
      : undefined;
  ...
  async getCapabilities(endpointId: string): Promise<RuntimeCapabilities> {
    ...
    if (!runtimeCapabilitiesUrl && !apiDomain)
      throw new Error("apiDomain not configured");
    if (!runtimeCapabilitiesUrl && !apiKey)
      throw new Error("apiKey not configured");

    const baseUrl =
      runtimeCapabilitiesUrl ||
      `https://${apiDomain}/api/v1/runtime/capabilities`;
    ...
    if (!response.ok) {
      ...
      const message =
        typeof err.error === "string"
          ? err.error
          : err.error?.message ||
            `Failed to get capabilities: ${response.status}`;
      throw new Error(message);
    }
  ...
  getSession(endpointId: string, mode?: SessionMode): SessionClient | null {
    if (mode)
      return (
        this._sessions.get(this._sessionKey(endpointId, mode))?.session ?? null
      );
    return (
      Array.from(this._sessions.values()).find(
  ...
  async readAloud(
    ...
    if (
      options.disableAutoPlay !== undefined ||
    ...
    const messageType =
      ...
      DEFAULT_READ_ALOUD_MESSAGE_TYPE;
    const payload = {
      messageType,
      text,
  ...
  private async _getDefaultReadAloudCapability(endpointId: string): Promise<{
    ...
    const capabilities = await this.assertEndpointCapability(
      endpointId,
      "audio_playback",
    );
  ...
  async startCall(options: StartCallOptions): Promise<SessionClient> {
    ...
    if (browserContext) {
      ...
      if (!sent) {
        logger.warn(
          "[DelphiClient] Failed to send browserContext before call dial",
          {
            endpointId,
          },
        );
  ...
  async upgradeToVoice(options: UpgradeToVoiceOptions): Promise<SessionClient> {
    ...
    if (!domain || !gw) {
      throw new Error(
        "Upgrade response missing TelPro / WebRTC gateway metadata",
      );
    }
  ...
  async sendDtmf(digit: string): Promise<void> {
    playDtmfTone(digit);
    this._setVoiceState({
      dtmfDigits: this._state.voiceCall.dtmfDigits + digit,
    });
  ...
  async reconnectCall(state: PersistedSessionState): Promise<void> {
    ...
    if (!this._state.voiceCall.registered) {
      logDebug(
        "Not registered yet — queuing reconnect until WebRTC + SIP are ready",
      );
      this._pendingReconnect = state;
      if (!this._state.voiceCall.initialized && !this._webrtc.initializing) {
        this._initWebRTCGateway(
          state.telproDomain,
          state.webrtcGatewayUrl,
        ).catch((error) => {
  ...
  cancelReconnect(): void {
    ...
    clearSessionState();
    this._setStatus(
      this._state.voiceCall.registered ? "Connected" : "Disconnected",
    );
    this._setVoiceState({ reconnecting: false });
    this._cleanupMedia();
  }
  ...
  private _touchSession(entry: SessionEntry): void {
    ...
    const timeoutMs =
      this._config.sessionIdleTimeoutMs ?? DEFAULT_IDLE_TIMEOUT_MS;
  ...
  private _resolveIceServers(
    telproDomain: string | null | undefined,
  ): RTCIceServer[] {
    ...
    if (!host) {
      logDebug(
        "ICE: no telproDomain — RTCPeerConnection with empty iceServers",
      );
      return [];
    }
   ...
   */
  private async _initWebRTCGateway(
    telproDomain: string,
    gatewayUrl: string,
  ): Promise<void> {
    ...
    if (gw.initializing && gw.initPromise) {
      logDebug(
        "WebRTC gateway already initializing, waiting for existing init",
      );
    ...
    if (!telproDomain) {
      throw new Error(
        "Cannot initialize WebRTC gateway without a telproDomain",
      );
    }
    if (!gatewayUrl) {
    ...
    const initPromise = (async () => {
      ...
      gw.gatewaySessionId =
        ((createResp["data"] as Record<string, unknown>)?.["id"] as number) ??
        null;
      ...
      gw.gatewayHandleId =
        ((attachResp["data"] as Record<string, unknown>)?.["id"] as number) ??
        null;
  ...
  private async _dial(attempt = 3): Promise<void> {
    ...
    const { registered, endpointId, sessionId, telproDomain } =
      this._state.voiceCall;
  ...
  private _handleGatewayMessage(msg: Record<string, unknown>): void {
    ...
    switch (eventType) {
      case "event": {
        const plugindata = msg["plugindata"] as
      ...
      case "error": {
        ...
        if (errMsg.includes("No such session")) {
          logger.warn(
            "Gateway session lost, tearing down to trigger reconnect",
          );
          this._teardownWebRTC();
        }
        break;
  ...
  private _handleSipEvent(
    ...
    const event =
      result?.["event"] ?? (data["sip"] !== "event" ? data["sip"] : undefined);
  ...
  private async _handleRemoteJsep(jsep: {
    type: string;
    sdp?: string;
  }): Promise<void> {
  ...
  private _handleRemoteVoiceHangup(): void {
    ...
    if (endpointId) {
      void this.endSession(endpointId, "voice_conversation").catch((error) => {
        logger.warn(
          "Failed to close voice session after remote hangup:",
          error,
        );
      });
    }
