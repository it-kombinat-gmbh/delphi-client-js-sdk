...
export class SessionClient {
  ...
  > = new Map();
  private _audioGroups: Map<
    string,
    { segments: BrowserAudioEvent[]; segmentCount?: number }
  > = new Map();
  ...
  private _cancelActiveAudio: (() => void) | null = null;
  ...
  connect(sessionId: string, wsToken: string, wsUrl?: string): void {
    ...
    const isNewSession =
      this._sessionId !== null && this._sessionId !== sessionId;
  ...
  sendContextUpdate(
    ...
    if (!this._sessionId) return false;
    const message = createContextUpdateMessage(
      this._sessionId,
      content,
      metadata,
    );
  ...
  setBrowserContext(browserContext: BrowserContext): boolean {
    if (!this._sessionId) return false;
    const message = createBrowserContextMessage(
      this._sessionId,
      browserContext,
    );
  ...
  sendActionProgress(
    ...
    if (!this._sessionId) return false;
    return this._sendRaw(
      createActionAckMessage(this._sessionId, actionId, status),
    );
  }
  ...
  stopAudioPlayback(): void {
    ...
    if (this._activeAudio) {
      try {
        ...
        if (typeof this._activeAudio.currentTime === "number") {
          this._activeAudio.currentTime = 0;
        }
        if (typeof this._activeAudio.src === "string") {
          this._activeAudio.src = "";
        }
      } catch {
        /* no-op: stopping playback should never throw into callers */
      }
    }
  ...
  private _connect(): void {
    if (!this._sessionId || !this._wsToken) {
      logger.debug(
        "[SessionClient] Missing sessionId or wsToken — cannot connect",
      );
      return;
    }
    ...
    if (message.streamId && this._sessionId) {
      ...
      try {
        sessionStorage.setItem(
          `channel_last_stream:${this._sessionId}`,
          message.streamId,
        );
      } catch {
        /* no-op */
  ...
  private _handleAudio(audio: AudioPayload, message: ChannelMessage): void {
    ...
    const current = this._audioBuffers.get(responseId) ?? {
      ...
      requestId: this._audioRequestId(audio),
      segmentIndex: this._audioSegmentIndex(audio),
      segmentCount: this._audioSegmentCount(audio),
      isFinal: this._audioIsFinal(audio),
    };
    ...
    if (audio.event === "delta") {
      ...
      current.requestId = this._audioRequestId(audio) ?? current.requestId;
      current.segmentIndex =
        this._audioSegmentIndex(audio) ?? current.segmentIndex;
      current.segmentCount =
        this._audioSegmentCount(audio) ?? current.segmentCount;
      current.isFinal = this._audioIsFinal(audio) ?? current.isFinal;
    ...
    if (audio.event === "done") {
      ...
      const browserEvent: BrowserAudioEvent = {
        ...
        ...(current.segmentIndex !== undefined && {
          segmentIndex: current.segmentIndex,
        }),
        ...(current.segmentCount !== undefined && {
      ...
      const completedEvent = this._completeAudioSegment(
        browserEvent,
        current.isFinal,
      );
      this._audioBuffers.delete(responseId);
      this._options.onAudio?.(completedEvent, message);
  ...
  private _completeAudioSegment(
    ...
    const isFinal =
      explicitFinal ??
      event.isFinal ??
      (event.segmentCount !== undefined && event.segmentIndex !== undefined
        ? event.segmentIndex >= event.segmentCount - 1
        : true);
    ...
    const group = this._audioGroups.get(event.requestId) ?? {
      segments: [],
      segmentCount: event.segmentCount,
    };
    if (event.segmentCount !== undefined)
      group.segmentCount = event.segmentCount;

    const existingIndex = group.segments.findIndex(
      (item) => item.responseId === event.responseId,
    );
    if (existingIndex >= 0)
      group.segments[existingIndex] = { ...event, isFinal };
    else group.segments.push({ ...event, isFinal });
    group.segments.sort(
    ...
    const completed =
      isFinal ||
      (group.segmentCount !== undefined &&
        group.segments.length >= group.segmentCount);

    if (!completed) {
      this._audioGroups.set(event.requestId, group);
      return { ...event, isFinal: false };
    }

    this._audioGroups.delete(event.requestId);
    return {
      ...event,
      isFinal: true,
      segmentCount:
        event.segmentCount ?? group.segmentCount ?? group.segments.length,
  ...
  private _resolveAudioWaiters(event: BrowserAudioEvent): void {
    const matched: number[] = [];
    const canResolveDefault = event.isFinal !== false;

    for (let i = 0; i < this._pendingAudioWaiters.length; i++) {
      const waiter = this._pendingAudioWaiters[i]!;
      if (waiter.responseId === event.responseId) {
        matched.push(i);
        continue;
      }
      if (
        waiter.responseId &&
        waiter.responseId === event.requestId &&
        canResolveDefault
      ) {
        matched.push(i);
      }
    ...
    if (matched.length > 0) {
      for (const i of matched.reverse()) {
        const [waiter] = this._pendingAudioWaiters.splice(i, 1);
        waiter?.resolve(event);
      }
      return;
    }

    if (canResolveDefault && this._pendingAudioWaiters.length > 0) {
      const index = this._pendingAudioWaiters.findIndex(
        (waiter) => !waiter.responseId,
      );
      if (index >= 0) {
        const [waiter] = this._pendingAudioWaiters.splice(index, 1);
        waiter?.resolve(event);
      }
    }
  }

  private _audioRequestId(audio: AudioPayload): string | undefined {
    return audio.requestId ?? stringMetadata(audio.metadata, "requestId");
  }

  private _audioSegmentIndex(audio: AudioPayload): number | undefined {
    return audio.segmentIndex ?? numberMetadata(audio.metadata, "segmentIndex");
  }

  private _audioSegmentCount(audio: AudioPayload): number | undefined {
    return audio.segmentCount ?? numberMetadata(audio.metadata, "segmentCount");
  }

  private _audioIsFinal(audio: AudioPayload): boolean | undefined {
