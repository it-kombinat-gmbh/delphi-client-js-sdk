...
export interface UseDelphiSessionOptions {
  ...
  /** Called when the SDK actually starts playing audio in the browser */
  onAudioPlaybackStart?: (audio: BrowserAudioEvent) => void;
  /** Called when browser audio playback finishes */
...
export interface UseDelphiSessionReturn {
  ...
  /** True while a read-aloud/browser-action request is in flight but playback has not started */
 ...
 * ```
 */
export function useDelphiSession(
  options: UseDelphiSessionOptions,
  ...
  return useMemo(() => {
    ...
    return {
      ...
      audioRequestPending: state.audioRequestPending,
